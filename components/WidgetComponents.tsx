...
export const HabitWidget: React.FC<WidgetProps> = ({ colors, isDarkMode }) => {
      ...
      <View style={{
        ...
        <Svg width={size} height={size} style={{ transform: [{ rotate: '-90deg' }] }}>
          ...
          <Circle
            ...
            cy={size / 2}
            r={radius}
            stroke={isDarkMode ? '#3A3A3C' : '#E5E5EA'}
  ...
  ), [size, strokeWidth, radius, strokeDasharray, strokeDashoffset, isDarkMode]);
  ...
  const DotComponent = React.memo(({ index }: { index: number }) => {
    const animatedDotStyle = useAnimatedStyle(() => {
      ...
      return {
        ...
        backgroundColor: isActive ? '#FF1744' : (isDarkMode ? '#48484A' : '#8E8E93'),
      };
    });
