...
export default function Question4Screen() {
  ...
  useFocusEffect(
    React.useCallback(() => {
      const loadSavedAnswer = async () => {
        try {
          const savedAnswer = await AsyncStorage.getItem('question-4-answer');
          if (savedAnswer !== null) {
  ...
  const handleOptionSelect = async (optionIndex: number) => {
    ...
    // Save the answer
    try {
      await AsyncStorage.setItem('question-4-answer', optionIndex.toString());
    } catch (error) {
      console.log('Error saving answer:', error);
    }
    
    setTimeout(() => {
      router.push('/question-5');
    }, 300);
  };
      ...
      <View style={styles.content}>
        ...
        <View style={styles.questionContainer}>
          <Text style={styles.questionText}>
            What's your ideal personality match?
          </Text>
        </View>
