...
export default function Question3Screen() {
  ...
  useFocusEffect(
    React.useCallback(() => {
      const loadSavedAnswer = async () => {
        try {
          const savedAnswer = await AsyncStorage.getItem('question-3-answer');
          if (savedAnswer !== null) {
  ...
  const handleOptionSelect = async (optionIndex: number) => {
    ...
    // Save the answer
    try {
      await AsyncStorage.setItem('question-3-answer', optionIndex.toString());
    } catch (error) {
  ...
  const options = [
    "Qur'an, Arabic, or fiqh topics",
    "Life, habits, and Islamic growth",
    "Deep talks about faith and struggles",
    "Anything real and meaningful"
  ];
      ...
      <View style={styles.content}>
        ...
        <View style={styles.questionContainer}>
          <Text style={styles.questionText}>
            Which types of convos would you enjoy most?
          </Text>
        </View>
