...
export default function Question9Screen() {
  ...
  useFocusEffect(
    React.useCallback(() => {
      const loadSavedAnswer = async () => {
        try {
          const savedAnswer = await AsyncStorage.getItem('question-9-answer');
          if (savedAnswer !== null) {
  ...
  const handleOptionSelect = async (optionIndex: number) => {
    ...
    // Save the answer
    try {
      await AsyncStorage.setItem('question-9-answer', optionIndex.toString());
    } catch (error) {
      console.log('Error saving answer:', error);
    }
    
    setTimeout(() => {
      router.push('/question-10');
    }, 300);
  };
      ...
      <View style={styles.content}>
        ...
        <View style={styles.questionContainer}>
          <Text style={styles.questionText}>
            What makes someone a great match for you?
          </Text>
        </View>
