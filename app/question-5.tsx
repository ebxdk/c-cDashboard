...
export default function Question5Screen() {
  ...
  useFocusEffect(
    React.useCallback(() => {
      const loadSavedAnswer = async () => {
        try {
          const savedAnswer = await AsyncStorage.getItem('question-5-answer');
          if (savedAnswer !== null) {
  ...
  const handleOptionSelect = async (optionIndex: number) => {
    ...
    // Save the answer
    try {
      await AsyncStorage.setItem('question-5-answer', optionIndex.toString());
    } catch (error) {
      console.log('Error saving answer:', error);
    }
    
    setTimeout(() => {
      router.push('/question-6');
    }, 300);
  };

  const options = [
    "Deep convos, no small talk",
    "Light-hearted and casual",
    "Depends on the person",
    "I like to listen more than speak"
  ];
      ...
      <View style={styles.content}>
        ...
        <View style={styles.questionContainer}>
          <Text style={styles.questionText}>
            How do you usually vibe with people?
          </Text>
        </View>
