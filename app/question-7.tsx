...
export default function Question7Screen() {
  ...
  useFocusEffect(
    React.useCallback(() => {
      const loadSavedAnswer = async () => {
        try {
          const savedAnswer = await AsyncStorage.getItem('question-7-answer');
          if (savedAnswer !== null) {
  ...
  const handleOptionSelect = async (optionIndex: number) => {
    ...
    // Save the answer
    try {
      await AsyncStorage.setItem('question-7-answer', optionIndex.toString());
    } catch (error) {
      ...
      <View style={styles.content}>
        ...
        <View style={styles.questionContainer}>
          <Text style={styles.questionText}>
            What kind of bonding moment sounds best?
          </Text>
        </View>
